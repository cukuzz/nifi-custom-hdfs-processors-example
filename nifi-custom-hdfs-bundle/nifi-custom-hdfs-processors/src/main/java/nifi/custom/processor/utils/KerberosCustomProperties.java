 ...
 * instance.
 */
public class KerberosCustomProperties {
	...
	/**
	 * Instantiate a KerberosProperties object but keep in mind it
	 * iskerberosConfigFile effectively a singleton because the krb5.conf file needs
	 * to be set as a system property which this constructor will take care of.
	 *
	 * @param kerberosConfigFile file of krb5.conf
	 */
	public KerberosCustomProperties(final File kerberosConfigFile) {
				...
				if (!kerberosConfigFile.canRead()) {
					return new ValidationResult.Builder().subject(subject).input(input).valid(false)
							.explanation(String.format(
									"unable to read Kerberos config [%s], please make sure the path is valid "
		...
		this.kerberosPrincipal = new PropertyDescriptor.Builder().name("Kerberos Principal").required(false)
				.description(
...
//				.expressionLanguageSupported(ExpressionLanguageScope.VARIABLE_REGISTRY)
		...
		this.kerberosKeytab = new PropertyDescriptor.Builder().name("Kerberos Keytab").required(false).description(
...
//				.expressionLanguageSupported(ExpressionLanguageScope.VARIABLE_REGISTRY)
	...
	public static List<ValidationResult> validatePrincipalAndKeytab(final String subject, final Configuration config,
			final String principal, final String keytab, final ComponentLog logger) {
		...
		if (isSecurityEnabled && blankPrincipal) {
			results.add(new ValidationResult.Builder().valid(false).subject(subject)
					.explanation("Kerberos Principal must be provided when using a secure configuration").build());
		}

		final boolean blankKeytab = (keytab == null || keytab.isEmpty());
		if (isSecurityEnabled && blankKeytab) {
			results.add(new ValidationResult.Builder().valid(false).subject(subject)
					.explanation("Kerberos Keytab must be provided when using a secure configuration").build());
		}
