...
package nifi.custom.processor.utils;
...
public abstract class AbstractHadoopCustomProcessor extends AbstractProcessor {
...
//            .build();
	...
	public static final PropertyDescriptor HDFS_URL = new PropertyDescriptor.Builder()
			.name("HDFS URL")
			.description("example: hdfs://127.0.0.1:9000")
			.required(true)
			.defaultValue("hdfs://127.0.0.1:9000")
			.addValidator(StandardValidators.NON_BLANK_VALIDATOR)
			.build();

	public static final PropertyDescriptor ACCESS_USER = new PropertyDescriptor.Builder()
			.name("Access User")
...
//			.identifiesControllerService(KerberosCredentialsService.class).required(false).build();
	...
	protected KerberosCustomProperties kerberosProperties;
	...
	// Holder of cached Configuration information so validation does not reload the
	// same config over and over
...
//		props.add(HADOOP_CONFIGURATION_RESOURCES);
	...
	protected KerberosCustomProperties getKerberosProperties(File kerberosConfigFile) {
		return new KerberosCustomProperties(kerberosConfigFile);
	}
...
//		final String explicitPrincipal = validationContext.getProperty(KerberosCustomProperties.getKerberosPrincipal()).evaluateAttributeExpressions().getValue();
//		final String explicitKeytab = validationContext.getProperty(KerberosCustomProperties.getKerberosKeytab()).evaluateAttributeExpressions().getValue();
...
//		}
	...
	public final void abstractOnScheduled(ProcessContext context) throws IOException {
		try {
			// This value will be null when called from ListHDFS, because it overrides all
			// of the default
...
//				resources = resetHDFSResources(configResources, context);
				resources = resetHDFSResources(context);
	...
	public final void abstractOnStopped() {
		...
		if (resources != null) {
			...
			// Clean-up the reference to the InstanceClassLoader that was put into
			// Configuration
			final Configuration configuration = resources.getConfiguration();
			configuration.setClassLoader(null);

			// Need to remove the Provider instance from the JVM's Providers class so that
			// InstanceClassLoader can be GC'd eventually
...
//	}
	...
	/*
	 * Reset Hadoop Configuration and FileSystem based on the supplied configuration
	 * resources.
	 */
//	HdfsResources resetHDFSResources(String configResources, ProcessContext context) throws IOException {
	HdfsResources resetHDFSResources(ProcessContext context) throws IOException {
		...
		final String hdfsUri = context.getProperty(HDFS_URL).getValue();
		final String accessUser = context.getProperty(ACCESS_USER).getValue();
		...
		config.set("hadoop.job.ugi", accessUser);

		// first check for timeout on HDFS connection, because FileSystem has a hard
		// coded 15 minute timeout
		checkHdfsUriForTimeout(config);

		// disable caching of Configuration and FileSystem objects, else we cannot
		// reconfigure the processor without a complete
		// restart
		...
		// -- use RESOURCE_LOCK to guarantee UserGroupInformation is accessed by only a
		// single thread at at time
		FileSystem fs;

		// set authorization. this processor access hdfs with this accessUser's user
		// name
		UserGroupInformation ugi = UserGroupInformation.createRemoteUser(accessUser);
...
//				ugi = SecurityUtil.loginSimple(config);
	...
	/**
	 * This method will be called after the Configuration has been created, but
	 * before the FileSystem is created, allowing sub-classes to take further action
	 * on the Configuration before creating the FileSystem.
	 *
	 ...
	 * take several minutes waiting for UDP packets to be received
	 *
	...
	/*
	 * Drastically reduce the timeout of a socket connection from the default in
	 * FileSystem.get()
	 */
	...
	/**
	 * Returns the relative path of the child that does not include the filename or
	 * the root path.
	 *
	...
	protected void tryKerberosRelogin(UserGroupInformation ugi) {
		try {
			getLogger().info(
					"Kerberos ticket age exceeds threshold [{} seconds] " + "attempting to renew ticket for user {}",
			...
			// Most likely case of this happening is ticket is expired and error getting a
			// new one,
...
//	}
	...
	/**
	 * Extending Hadoop Configuration to prevent it from caching classes that can't
	 * be found. Since users may be adding additional JARs to the classpath we don't
	 * want them to have to restart the JVM to be able to load something that was
	 * previously not found, but might now be available.
	 *
