...
@SeeAlso({ WriteHDFS.class })
public class ReadHDFS extends AbstractHadoopCustomProcessor {

	@Deprecated
	public static final PropertyDescriptor DISTRIBUTED_CACHE_SERVICE = new PropertyDescriptor.Builder()
			.name("Distributed Cache Service")
			.description("This property is ignored.  State will be stored in the " + Scope.LOCAL + " or "
					+ Scope.CLUSTER + " scope by the State Manager based on NiFi's configuration.")
			.required(false)
	...
	static final AllowableValue FILTER_DIRECTORIES_AND_FILES_VALUE = new AllowableValue(
			FILTER_MODE_DIRECTORIES_AND_FILES, "Directories and Files",
	...
	static final AllowableValue FILTER_FILES_ONLY_VALUE = new AllowableValue(FILTER_MODE_FILES_ONLY, "Files Only",
			"Filtering will only be applied to the names of files.  If " + RECURSE_SUBDIRS.getName()
					...
					+ "the regular expression defined in " + FILE_FILTER.getName() + ".");
	static final AllowableValue FILTER_FULL_PATH_VALUE = new AllowableValue(FILTER_MODE_FULL_PATH, "Full Path",
	...
	public static final PropertyDescriptor FILE_FILTER_MODE = new PropertyDescriptor.Builder().name("file-filter-mode")
			.displayName("File Filter Mode")
			.description("Determines how the regular expression in  " + FILE_FILTER.getName()
	...
	public static final PropertyDescriptor MIN_AGE = new PropertyDescriptor.Builder()
			...
			.required(false).addValidator(StandardValidators.createTimePeriodValidator(0, TimeUnit.MILLISECONDS,
	...
	/**
	 * Determines which of the given FileStatus's describes a File that should be
	 * listed.
	 *
	 * @param statuses the eligible FileStatus objects that we could potentially
	 *                 list
	...
	Set<FileStatus> determineListable(final Set<FileStatus> statuses, ProcessContext context) {
		...
		final Long minAgeProp = context.getProperty(MIN_AGE).asTimePeriod(TimeUnit.MILLISECONDS);
		// NIFI-4144 - setting to MIN_VALUE so that in case the file modification time
		// is in
		...
		if (orderedEntries.size() > 0) {
			...
			// another iteration has occurred without new files and special handling is
			// needed to avoid starvation
			if (latestListingTimestamp == minTimestamp) {
				// We are done if the latest listing timestamp is equal to the last processed
				// time,
				...
				// Otherwise, newest entries are held back one cycle to avoid issues in writes
				// occurring exactly when the listing is being performed to avoid missing data
	...
	public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
		// We have to ensure that we don't continually perform listings, because if we
		// perform two listings within
		// the same millisecond, our algorithm for comparing timestamps will not work.
		// So we ensure here that we do
		// not let that happen.
		...
		// Ensure that we are using the latest listing information before we try to
		// perform a listing of HDFS files.
		try {
			if (resetState) {
				getLogger().debug(
						"Property has been modified. Resetting the state values - listing.timestamp and emitted.timestamp to -1L");
			...
			getLogger().error(
					"Failed to retrieve timestamp of last listing from the State Manager. Will not perform listing until this is accomplished.");
			context.yield();
			return;
		}
	...
	private Set<FileStatus> getStatuses(final Path path, final boolean recursive, final FileSystem hdfs,
			final PathFilter filter, String filterMode) throws IOException, InterruptedException {
		...
		if (isPostListingFilterNeeded(filterMode)) {
			// For this filter mode, the filter is not passed to listStatus, so that
			// directory names will not be
			// filtered out when the listing is recursive.
			statuses = getUserGroupInformation()
					.doAs((PrivilegedExceptionAction<FileStatus[]>) () -> hdfs.listStatus(path));
		} else {
			statuses = getUserGroupInformation()
		...
		for (final FileStatus status : statuses) {
			if (status.isDirectory()) {
				...
				if (isPostListingFilterNeeded(filterMode)) {
					// Filtering explicitly performed here, since it was not able to be done when
					// calling listStatus.
	...
	/**
	 * Determines if filtering needs to be applied, after calling
	 * {@link FileSystem#listStatus(Path)}, based on the given filter mode. Filter
	 * modes that need to be able to search directories regardless of the given
	 * filter should return true. FILTER_MODE_FILES_ONLY and FILTER_MODE_FULL_PATH
	 * require that {@link FileSystem#listStatus(Path)} be invoked without a filter
	 * so that all directories can be traversed when filtering with these modes.
	 * FILTER_MODE_DIRECTORIES_AND_FILES should return false, since filtering can be
	 * applied directly with {@link FileSystem#listStatus(Path, PathFilter)}
	 * regardless of a recursive listing.
	 * 
